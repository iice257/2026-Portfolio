...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(targetCanvas) {
      ...
      if (!isWebGL2) {
        webgl = targetCanvas.getContext('webgl', params) || targetCanvas.getContext('experimental-webgl', params);
      }
      if (!webgl) {
        return { gl: null, ext: {} };
      }
    ...
    function initFramebuffers() {
      ...
      const { formatRGBA, formatRG, formatR } = ext;
    ...
    function handleMouseDown(event) {
      ...
      updatePointerDownData(pointer, -1, point.x, point.y);
      clickSplat(pointer);
    }
    ...
    function handleTouchStart(event) {
      ...
      for (let i = 0; i < touches.length; i += 1) {
        const point = getLocalPoint(touches[i].clientX, touches[i].clientY);
        if (point) updatePointerDownData(pointer, touches[i].identifier, point.x, point.y);
      }
    }
    ...
    function handleTouchMove(event) {
      ...
      for (let i = 0; i < touches.length; i += 1) {
        const point = getLocalPoint(touches[i].clientX, touches[i].clientY);
        if (point) updatePointerMoveData(pointer, point.x, point.y, pointer.color);
      }
    }
