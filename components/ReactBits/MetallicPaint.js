...
export default function MetallicPaint({
  ...
  useEffect(() => {
    if (!isVisible || !initGL()) return undefined;
    setReady(true);
  ...
  }, [initGL, isVisible, resizeCanvas]);

  useEffect(() => {
    if (!isVisible || !ready || !imageSrc) return undefined;

    setTextureReady(false);
    if (imageDataCache.has(imageSrc)) {
      uploadTexture(imageDataCache.get(imageSrc));
  ...
  }, [imageSrc, isVisible, ready, uploadTexture]);
